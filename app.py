...
mydb=connection.MySQLConnection(user='root',host='localhost',password='reshma',db='ecom')
@app.route('/')
def home():
...
def adminprofile():
    if session.get('admin'):
        try:
            cursor=mydb.cursor(buffered=True)
            cursor.execute('select username,address,ph_no,profilepic from admindata where adminemail=%s',[session.get('admin')])
...
def addreview(itemid):
    if request.method == 'POST':
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('insert into reviews(review,rating,itemid,user)values(%s,%s,uuid_to_bin(%s),%s)',[review,rating,itemid,session.get('over')])
        mydb.commit()
        cursor.close()
    ...
    if request.method=='POST':
        ...
        if MATCHING_STRG.search(sdata):
            try:
                ...
                print(e)
                flash('couldnt fetch search details')
                return redirect(url_for('index'))
            else:
                return render_template('dashboard.html',items_data=items)
        else:
